...
# # Based on the images, here is the reconstructed Python code. This script seems to be a collection of functions for causal inference, likely within a module or a notebook, with a focus on **AIPW (Augmented Inverse Probability Weighting)** and related variance calculations.
...
#     解析的なOCD (平均因果効果) を計算
#     """
#     g = np.arange(1, num_strata + 1)
#     return np.mean(g * (1 + np.exp(-strata)) / strata)

# def get_rorr_estimates(data, f_t, g_x, h_x):
#     """
#     残差 on 残差の回帰 (RORR) を実行 (ただし, OLSと限定的)
#     """
#     y_hat = data.y - g_x(data.x)
#     t_hat = data.t - h_x(data.x)
    
#     fit = OLS(y_hat, add_constant(t_hat)).fit()
#     coef = fit.params.loc['t']
#     se = fit.bse.loc['t']
#     lwr, upr = fit.conf_int().loc['t']
...
# def make_m_tx(model):
#     """
#     Makes a function for the outcome model.
#     model: sklearn-style model (must implement .predict)
#     Returns a function that applies the model.
#     """
#     def m_tx(t_array, x_array):
#         X_array = np.column_stack([x_array, t_array])
...
#     m_vals = [f_t(t) for t in t_vals]
#     pscore = np.array([poisson.pmf(t, h_x_vals) for t in t_vals]).T
    
#     if pscore.shape[1] != len(t_vals):
#         raise ValueError("p_model must return probabilities for each t in t_vals")
    
#     influence = np.zeros(pscore.shape)
#     for i, t in enumerate(t_vals):
#         indicator = (t_obs == t).astype(float)
#         influence[:, i] = indicator / np.clip(pscore[:, i], a_min=1e-12, a_max=None) * (y - m_vals[i]) + m_vals[i] - g_x_vals
        
#     influence_df = pd.DataFrame(influence, index=idx, columns=[f"f_t_{t}" for t in t_vals])
...
#     """
#     aipw_matrix = compute_aipw_matrix(df, f_t, g_x, h_x).values
#     T_max = aipw_matrix.shape[1] - 1
...
# def estimate_weighted_increments_v2(df, f_t, g_x, h_x):
#     """
#     Estimates weighted increments between AIPW estimates for each value of t.
...
#     1. Computes empirical proportions of each value of t (used as weights)
#     2. Computes AIPW influence function matrix
#     3. Computes the difference in mean influence across consecutive t values
#     4. Returns weighted differences using empirical t distribution
