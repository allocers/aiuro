...
#     """
#     Three vertically stacked plots:
#     1. f(x) = log(x+1) plus tone*tangent line for each of T and ω*T.
#        The slope of each tangent is the average derivative E[1/(X+1)].
#        The tangent point x0 satisfies 1/(x0+1) = E[1/(X+1)].
#     2. Histogram of T with a vertical line at mean(T).
#     3. Histogram of ω*T with a vertical line at mean(ω*T).
...
#     data = df
#     t = data['t'].to_numpy()
#     t_star = data['t_star'].to_numpy()
#     wt_star = (data['weight'] * data['t_star']).to_numpy()
    
#     mean_T = np.mean(t)
#     mean_t_star = np.mean(t_star)
#     mean_wt_star = np.mean(wt_star)
    
#     # --- helper: compute slope and tangent point
#     # returns (slope, x0, f(x0)) for the average derivative tangent.
#     def slope_and_anchor(values, weights=None):
#         if weights is None:
#             weights = np.ones(len(values))
            
#         slope = np.mean(1.0 / (values + 1) * weights) / np.mean(weights)
#         x0 = 1.0 / slope - 1.0 # solve 1/(x0+1) = slope
#         y0 = np.log(x0 + 1)
#         return slope, x0, y0
...
#     axs[0].plot(x, np.log(x + 1), color='black', label="$f(x) = \log(x+1)$")
    
#     axs[0].plot(x, tangent_T, color=color_T, linestyle='--',
#                 label=f"E[f'(T)]")
    
#     axs[0].plot(x, tangent_wt, color=color_wt_star, linestyle='--',
#                 label=f"E[f'(ω*t*)]")
...
# # The function call and savefig commands are not part of the function definition
# # but were shown in a previous image.
