...
# def f(t_array):
#     """
#     The g function in Y = f(t) + g(x) + e^a
#     """
#     return np.log(t_array + 1)
...
#     rng = np.random.default_rng(seed)
#     e = rng.normal(loc=0.0, scale=1.0, size=len(t_array))
#     return f(t_array) + g_x(x_array) + e
...
# # Incremental Effect (処置を1増加させたときの差分効果)
# def append_incremental(df):
#     """
#     Adds log((t + 2) / (t + 1)), the effect of incrementing t, to the input DataFrame
...
#     # h(x) + u のため
#     df['weight'] = (t - x) ** 2 / np.mean((t - x) ** 2) # 式(10)の重み部分の定義
#     return df

# def simulate_dataset(sample_size, num_strata, seed=None):
#     x = generate_x(sample_size, num_strata, seed=None)
#     t = generate_t(x, seed)
#     y = generate_y(t, x, seed)
#     data = pd.DataFrame({'y': y, 't': t, 'x': x})
...
#         .pipe(append_t_star)
#         .pipe(append_conditional_variance_weight)
#         .assign(p_lm=lambda df: df.weight * 1 / (df.t_star + 1)) # 式(10)参照
#     )
    
#     return data

def generate_t_binary(x_array, seed=None):
    rng = np.random.default_rng(seed)
    p = h_x(x_array)
    p_trans = 1 / (1 + np.exp(-p))
    return rng.binomial(1, p_trans)

def simulate_dataset_binary(sample_size, num_strata, seed=None):
    x = generate_x(sample_size, num_strata, seed=None)
    t = generate_t_binary(x, seed)
    y = generate_y(t, x, seed)
    data = pd.DataFrame({'y': y, 't': t, 'x': x})
