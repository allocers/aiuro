...
# --- ダミー関数定義 (estimators_2.pyが見つからない場合に使用) ---
...
def simulate_dataset_dummy(N, num_cov, num_strata, seed):
    ...
    df['t'] = np.random.randint(0, 2, N)
    df['y'] = np.random.rand(N)
    df['y_pre'] = np.random.rand(N)
    df['strata'] = pd.qcut(df['y_pre'], num_strata, labels=False, duplicates='drop')
    return df

def get_aipw_matrix_dummy(df, outcome_model, pscore_model, t_max):
    return pd.DataFrame(np.random.rand(len(df), t_max + 1))

def check_balance_dummy(df, pscore_model):
    plt.figure(figsize=(10, 6))
    plt.scatter([0.2, 0.4, 0.6], [1, 2, 3], c='orange', s=100, label='Before IPW')
    plt.scatter([0.05, -0.05, 0.02], [1, 2, 3], c='blue', s=100, label='After IPW')
    ...
    plt.title('Balance in Pre-Treatment Outcome After Weighting (Dummy Data)')
    plt.legend()
    plt.show()
    return 0.5, 0.05, pd.Series(np.random.rand(len(df)))

# estimators_2.pyが存在するか確認
try:
    import estimators_2 as est2
except ImportError:
    print("Warning: 'estimators_2' not found. Using dummy functions.")
    est2 = type('est', (), {
        'get_rorr_estimates': get_rorr_estimates_dummy,
        'estimate_aipw_matrix': estimate_aipw_matrix_dummy,
        'simulate_dataset': simulate_dataset_dummy,
        'get_aipw_matrix': get_aipw_matrix_dummy, # 追加
        'check_balance': check_balance_dummy # 追加
    })
